...
class uTorrent(genericDownloader):
    def __init__(self, name, minVersion, url, username, password):
        ...
        self.MissingFileErrorMessage = 'DownloadClientuTorrentTorrentStateMissingFiles'
    ...
    async def InstanceCheck(self):
        ...
        try: 
            ...
            logger.error('!! %s Error: !!', 'uTorrent')
            logger.error('> %s', error)
            logger.error('> Details:')
            logger.error(response.text)
            return True

        # check uTorrent version
        ...
        if version.parse(qbit_version) < version.parse(self.minVersion):
            logger.error('-- | %s *** Error: %s ***', 'uTorrent', 'Please update uTorrent to at least version %s Current version: %s',self.minVersion, qbit_version)
            return True

        logger.info('OK | %s', 'uTorrent')
        logger.debug('Current version of %s: %s', 'uTorrent', qbit_version) 
        return False
    ...
    async def CreateProtectionTag(self, tag):
        ...
        if not tag in current_tags:
            logger.info('Creating tag in uTorrent: %s', tag)  
            if not TEST_RUN:
    ...
    def IsMetaMissing(self, queueItem):
        return queueItem['status'] == 'queued' and queueItem['errorMessage'] == 'uTorrent is downloading metadata'
